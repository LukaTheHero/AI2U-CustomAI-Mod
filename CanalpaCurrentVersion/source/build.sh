...
# Private features are canak's only (02-USER-PREFERENCES.md rule 5b): developer
# cheats and Canalpa mode. They are compiled in by default because every local
# build wants them, and excluded by --release so the shipped DLL cannot contain
# the code even by accident.
#
# Compile-time, not a runtime toggle, and the distinction matters: a config flag
# would leave the code in the binary for anyone to find, flip or datamine. The
# Nexus build should not merely default these off - it should not have them.
PRIVATE_SRC="Cheats.cs Canalpa.cs"
PRIVATE_DEF="-define:CHEATS;CANALPA"
if [ "$RELEASE" = "1" ]; then
  PRIVATE_SRC=""
  PRIVATE_DEF=""
  echo "==> RELEASE build: developer cheats and Canalpa mode excluded from the binary"
fi
...
"$CSC" -nologo -noconfig -target:library -optimize+ -nostdlib+ \
  ...
  Lore.cs Bios.cs Voices.cs $PRIVATE_SRC 2>&1 | grep -v 'warning CS1701\|previous warning' || true
