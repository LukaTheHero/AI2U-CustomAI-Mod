...
namespace AI2UCustomAI
{
    ...
    public static class ModUiPatch
    {
            ...
            if (apply != null)
            {
                ...
                if (go != null)
                {
                    ...
                    if (_vTest != null)
                    {
                        ...
                        _vTest.onClick.AddListener(delegate { page.StartCoroutine(RunVoiceTest()); });
                    }
        ...
        static IEnumerator RunVoiceTest()
        {
            if (_vTest != null) _vTest.interactable = false;
            SetVoiceLabel("Testing...", Color.white);
            SaveVoiceFields();

            if (string.IsNullOrEmpty(Plugin.CfgGrokApiKey.Value))
            {
                SetVoiceLabel("No key", Color.red);
            ...
            if (clip == null)
            {
                SetVoiceLabel(GrokTts.FailureLabel(), Color.red);
            ...
            else
            {
                ...
                SetVoiceLabel("Works!", Color.green);
        ...
        static void BuildTestButton(UIManager_APIKeyPage page, Button apply)
        {
            ...
            if (_test != null)
            {
                ...
                _test.onClick.AddListener(delegate { page.StartCoroutine(RunTest()); });
            }
    ...
    public static class GrokVoiceDropdownPatch
    {
        ...
        static void Build(UIManager_Audio page)
        {
            ...
            {
                bool on = v == 1;
                Plugin.CfgGrokEnabled.Value = on;
                Plugin.SaveCfg();

                if (on && string.IsNullOrEmpty(Plugin.CfgGrokApiKey.Value))
                    Plugin.Log.LogWarning("AI Voice switched on, but no TTS key is set; "
