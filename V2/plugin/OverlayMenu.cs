...
namespace AI2UCustomAI
{
    internal static class OverlayMenu
    {
        ...
        static void DrawWindow(int id)
        {
            ...
            Bool(Plugin.CfgClampValues, "Clamp actions to the game's real action list" + ModTag);

            Header("Memory");
            IntRow("HistoryMaxTokens", "Max memory tokens" + ModTag);
            ...
            Bool(Plugin.CfgBlockGameAi,
                "Block the game's dialogue AI calls while the mod is on (play, fetchAsync)" + ModTag);
            Bool(Plugin.CfgBlockGameExtras,
                "Also block summary / envision / memorize" + ModTag);
            ...
            Bool(Plugin.CfgSpeakActions, "Read *actions* aloud too (off: speak only her words)" + ModTag);
        ...
        // Marks the options that change how the game itself plays, as opposed to
        // the ones that only point the mod at a different endpoint or voice.
