...
Released 2026-08-08. Baseline is V3.0. Verified working on both the Steam and
itch.io builds with the mod switched off, which is the case 3.1.1 exists to fix.
...
**And the half the first attempt missed: launching already-off.** Restoring on the
toggle only helps someone who flips it. Anyone arriving from 3.1.0 already has the
stale `LocalTTS = 1` in their registry, so `Awake` reads it back *before* any
toggle can run, `Postfix` early-outs on the disabled switch, `Restore()` is never
reached, and Steam is silent out of the box — blamed on the game, with nothing in
the mod's own settings to explain it.

So `Postfix` no longer simply returns when the mod is off. It clears a leftover
override first: `isLocalSpeak = false`, `LocalTTS = 0`, saved, with a log line
naming what happened. Scoped to builds that genuinely cannot synthesize locally —
where the local path works, that flag is a legitimate user preference and
overriding it would be the mod meddling while switched off.
...
### Known issue: the update check points at a repo that does not exist

`LATEST_VERSION_URL` is a placeholder
(`raw.githubusercontent.com/canak/AI2UCustomAI/main/VERSION.txt`) and returns 404,
which the log shows as `Version check failed: HTTP/1.1 404 Not Found`. Harmless in
itself, but the failure mode is wrong: `LatestVersion` stays null, so the panel
renders a green *"up to date"* regardless — it would claim that even when a real
update existed. Either point it at a repo with a `VERSION.txt`, or render a failed
check neutral with "(check failed)" rather than a false green.
