...
namespace AI2UCustomAI
{
    internal static class OverlayMenu
    {
        ...
        static void DrawWindow(int id)
        {
            ...
            {
                bool outdated = Plugin.LatestVersion != null && Plugin.LatestVersion != Plugin.VERSION;
                GUIStyle versionStyle = new GUIStyle(GUI.skin.label);
                versionStyle.fontSize = 11;
                versionStyle.alignment = TextAnchor.UpperRight;
                versionStyle.normal.textColor = outdated ? new Color(1f, 0.4f, 0.4f) : new Color(0.4f, 0.9f, 0.4f);
                versionStyle.fontStyle = outdated ? FontStyle.Bold : FontStyle.Normal;

                string versionText = outdated
                    ? "Mod v" + Plugin.VERSION + " (update: " + Plugin.LatestVersion + ")"
            ...
            Bool(Plugin.CfgClampValues, "Clamp actions to the game's real action list");
