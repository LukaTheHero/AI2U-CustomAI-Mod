...
//   safe code, which room was generated, the potion recipe, the ship's systems.
//
    ...
    internal static class Lore
    {
        ...
        static string PersonaTerm()
        {
            ...
            if (lv == 99)
            {
                ...
                // Evie is the one exception to the per-character pattern. There is
                // no "L99StoryGuide_IRLGirl" term - her authored persona lives on
                // the wake-up encounter instead, and formatting the name like
                // everyone else's produced a term that does not exist, so she was
                // the single character in the game left with no persona at all
                // while the other six worked. Verified against resources.assets:
                // L99StoryGuide_ exists for Eddie, Elysia, Estelle, Eiona and
                // ParrotGirl, and for nobody else.
                if (who == "IRLGirl") return "StoryGuide/L99WakeUpIRLGirlDialogue_SG";

                if (who != null) return "StoryGuide/L99StoryGuide_" + who;
            }
            return null;
        }
        ...
        static void Surroundings(StringBuilder sb)
        {
            ...
            for (int i = 0; i < entries.Count && lines.Count < 40; i++)
            {
                ...
                if (name == null || ai == null) continue;
                lines.Add("- " + Spaced(name.Replace('_', ' ')) + ": " + ai);
            }
