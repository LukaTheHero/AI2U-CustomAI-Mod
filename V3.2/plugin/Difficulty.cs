// The difficulty slider: how hard she is to win, as a disposition.
//
// Four positions - Easy, Normal, Hard, Masochist - and Normal is the base game,
// enforced structurally: Block() returns null on Normal, so not one token about
// difficulty enters the request and the prompt is byte-identical to the slider
// not existing. The game's own engine arithmetic (trust caps, patience, the
// favorability table) is untouched on every setting; what changes is HER - how
// she reads people, what she forgives, what she remembers, and what it takes to
// convince her. That is exactly the layer a language model actually controls,
// which is why this is a prompt block and not a Harmony patch.
//
...
//   refuses everything, which is not difficulty, it is a wall. The instruction
//   is that perfection or sincerity works and nothing else does.
//
//   Disposition, not lore. Nothing here tells her a fact about her world, so
//   the never-invent rule is not in play. It tells her what kind of judge she
//   is. The engine values she may use (favorability words, angry levels) are
//   the same legal sets as always - GameVocab still owns those.
//
// Composes with everything: Canalpa gates still need the same trust, the
// high-risk multiplier still scales whatever a turn was worth, and OOC still
// overrides persona per tagged turn. On Hard and Masochist the game's own
// thresholds mean her sharper temper is genuinely more dangerous - that is the
// point, and the panel says so.
using System;
using System.Text;
