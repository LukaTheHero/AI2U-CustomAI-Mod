...
namespace AI2UCustomAI
{
    internal static class Cheats
    {
        ...
        // "One of everything you carry", on a button instead of F3.
        //
        // GiftYourselfAnything watches F3 from InventoryPlayerMonitor.Update()
        // and calls its own PlusOne() on tap or on a held threshold. Its Update()
        // reads nothing but F3 - verified against the shipped IL, which contains
        // exactly three Input calls (GetKeyDown/GetKey/GetKeyUp) all on KeyCode
        // 284 and no other key - so skipping the whole method suppresses the key
        // and costs nothing else. That is why this prefixes Update() rather than
        // trying to filter the key: there is no other behaviour in there to keep.
        //
        // F3 is worth taking off the keyboard because it fires while a chat box
        // has focus and duplicates the player's whole inventory silently. A panel
        // button makes it deliberate.
        public static bool GiftOneOfEach()
        {
            try
            {
                object monitor = FindMonitor();
                if (monitor == null)
                {
                    Plugin.Log.LogWarning("Cheats: nothing to gift - the inventory monitor is not in "
                        + "this scene yet. Load a level with an inventory first.");
                    return false;
                }

                // Its own method, so slot rules, sprites and the AI-gift flag stay
                // that mod's business rather than being reimplemented here.
                Traverse plus = Traverse.Create(monitor).Method("PlusOne");
                if (!plus.MethodExists())
                {
                    Plugin.Log.LogWarning("Cheats: GiftYourselfAnything is installed but PlusOne() is "
                        + "missing, so its layout changed. Not guessing at a replacement.");
                    return false;
                }

                plus.GetValue();
                Plugin.Log.LogInfo("Cheats: added one of everything currently carried.");
                return true;
            }
            catch (Exception e)
            {
                Plugin.Log.LogWarning("Cheats: could not gift items: " + e.Message);
                return false;
            }
        }

        // The monitor is added to the player's Inventory_Player object by that
        // mod's own Harmony postfix, so it only exists once a level with an
        // inventory has loaded. Found by name to avoid a hard reference to an
        // assembly that may not be installed.
        static object FindMonitor()
        {
            try
            {
        ...
        // True when the button can actually do something, so the panel can grey
        // the row in a menu scene instead of offering a button that only logs.
        //
        // Throttled because this is read from the draw loop and FindObjectsOfType
        // walks every loaded object: OnGUI runs several times per frame (once per
        // event), so an unthrottled scan here would be one of the most expensive
        // things the panel does, to answer a question that changes only on scene
        // load. Twice a second is far quicker than a player can react.
        static float _giftProbeAt = -99f;
        static bool _giftReady;
        ...
        public static bool GiftReady
        {
            get
            {
                ...
                if (now - _giftProbeAt >= 0.5f)
                {
                    _giftProbeAt = now;
                    _giftReady = FindMonitor() != null;
                }
                return _giftReady;
            }
        }
