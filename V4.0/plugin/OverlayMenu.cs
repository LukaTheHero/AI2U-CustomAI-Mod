...
namespace AI2UCustomAI
{
    internal static class OverlayMenu
    {
        ...
        static void DrawWindow(int id)
        {
            ...
            Bool(Plugin.CfgGameServerTts, "Use's game's servers for TTS while mod's tts is turned off. "
                + "(if this is turned off she will be silent when mod's tts is turned off)");
            ...
            if (Plugin.CfgShowAdvanced.Value)
            {
                ...
                if (Canalpa.Active)
                {
                    ...
                    DangerNote("  Each of these is a door the base game gives her no way to offer, "
                        + "however close you get. With them on she can decide to do it herself once "
                        + "she fully trusts you, above the game's own FullyTrust mark of 40. Every one "
                        + "fires the game's own event, so the animations, mission goals, achievements "
                        + "and her authored reactions all happen exactly as normal. She is told it is "
                        + "her choice, not an instruction, and she is free to refuse.");
                    DangerNote("  The trust requirement is deliberately not adjustable: below it the "
                        + "game's own code answers an opening door by starting the final chase or "
                        + "docking trust, so a lower threshold would not be a harder unlock, it would "
                        + "be a death.");
                    DangerNote("  Trust alone is not enough. Past the trust mark she first spends a "
                        ...
                        + "before any of it becomes something she can offer at all.");
                    ...
                    {
                        DangerNote("  These are the game's own shipped endings - the plushie in the "
                            + "cabin, caught in the apartment, and each level's equivalent. This only "
                            + "lets you reach one on purpose instead of stumbling into it.");
                        DangerNote("  She is never told she can do this and is never watching for it. "
                            + "Until YOU say it plainly, in your own words, the ability does not exist "
                            + "as far as she knows - nothing about it is sent to the model at all. "
                            + "Being affectionate cannot start it: \"I never want to leave you\" is a "
                            + "loving thing to say here and is treated as one.");
                        DangerNote("  To start it you have to name it - being killed, being kept, "
                            + "never going home - not merely say you want to stay. Then she has to "
                            + "spell out what it means, at least " + Consent.MinTurnsNeeded + " turns "
                            + "have to pass, and you have to name it AGAIN with a clear yes. A bare "
                            + "\"yes\" will not do it, so a throwaway line cannot be finished by one "
                            + "later. Any 'wait', 'no', 'maybe' or 'just kidding' withdraws all of it.");
                        DangerNote("  She is also told she did not ask for this, that it is a strange "
                            + "thing to be asked, and that refusing is a perfectly good answer - the "
                            + "more she loves you the more she is likely to say no. You are meant to "
                            + "have to convince her, and she may simply not agree.");
                    }
        ...
        static ConfigEntryBase[] AdvancedEntries()
        {
            return new ConfigEntryBase[]
            {
                ...
                Plugin.CfgCanalpaBasement, Plugin.CfgCanalpaHiddenIsland,
                Plugin.CfgCanalpaWillingEnd,
#endif
        ...
        static void CanalpaReadinessRow()
        {
            ...
            if (rows.Count == 0)
            {
                ...
                GUILayout.Label("  Nothing of hers is unlockable in this level.", val);
                return;
            }
        ...
        static void GiftRow()
        {
            PlainNote("  Gift yourself anything: installed. Type an item name in the chat box, then hold "
                + "either Ctrl and press Enter to receive it instead of sending it.");

            // Its F3 is suppressed by GiftKeySuppressPatch and replaced by this
            // button. Said plainly because that mod's own readme documents F3, so
            // the key doing nothing would otherwise look like a broken install.
            PlainNote("    Its F3 hotkey is switched off, so this button is the only way to do it. F3 "
                + "fired while the chat box had focus and could duplicate your inventory mid-sentence.");

            GUILayout.BeginHorizontal();
            GUILayout.Space(20f);

            // Greyed outside a level: the monitor that owns the item logic is
            // attached to the player's inventory by that mod, so in a menu scene
            // there is genuinely nothing to add to and the button would only log.
            bool ready = Cheats.GiftReady;
            ...
            if (GUILayout.Button("One of everything I carry", GUILayout.Width(200f)))
                Cheats.GiftOneOfEach();
