...
# The developer cheats stay canak's only (02-USER-PREFERENCES.md rule 5b).
# Compiled in by default because every local build wants them, and excluded by
# --release so the shipped DLL cannot contain the code even by accident.
#
...
# Nexus build should not merely default the cheats off - it should not have them.
PRIVATE_SRC="Cheats.cs"
PRIVATE_DEF="-define:$CANALPA_DEF;CHEATS"
if [ "$RELEASE" = "1" ]; then
  PRIVATE_SRC=""
  PRIVATE_DEF="-define:$CANALPA_DEF"
  echo "==> RELEASE build: developer cheats excluded (Canalpa mode ships, off by default)"
fi
...
"$CSC" -nologo -noconfig -target:library -optimize+ -nostdlib+ \
  ...
  Lore.cs Bios.cs Voices.cs Mechanics.cs $CANALPA_SRC $PRIVATE_SRC 2>&1 | grep -v 'warning CS1701\|previous warning' || true
