...
namespace AI2UCustomAI
{
    internal static class Canalpa
    {
        ...
        // The ending alone keeps a hard trust floor on top of Consent. canak's
        // 4.2.2 redesign removed the code gates from her SECRETS; the ending is
        // not a secret, it is the irreversible act, and its gates were
        // explicitly out of scope. The floor lands her reaction in the authored
        // FullyTrust branches rather than the hostile ones.
        static bool TrustReady(Act a)
        {
            ...
            {
                float? t = TrustNow();
                return t.HasValue && t.Value > FullyTrust;
            }
            catch (Exception) { return false; }
        }
        ...
        static bool Available(Act a)
        {
            ...
            return TrustReady(a) && Consent.Pending;
        }
        ...
        static string EndingBlock()
        {
            ...
            if (a == null || !TrustReady(a)) return null;
