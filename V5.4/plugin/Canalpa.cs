...
namespace AI2UCustomAI
{
    internal static class Canalpa
    {
        ...
        // How many clearance steps SHE has granted on this level. Kept for the
        // log and the panel only - it is no longer a gate.
        //
        // History, because this line has now been wrong twice in two different
        // ways. The first version keyed "done" on BonusSecurityLevel >= 2, which
        // the player's own repairs also raise - fixing two ship systems silently
        // disabled her. The fix kept a cap of two but counted only HER grants,
        // which looked principled and produced this live failure (2026-08-16):
        // she granted twice early, the act went "already done", the offer block
        // stopped mentioning it, and when the player asked over OOC - a channel
        // sworn to literal truth - she truthfully answered that she had no way
        // to raise clearance at all. An invisible allowance turned into her
        // denying a capability she has.
        //
        // So the cap is gone, on the same reasoning that removed every other
        // coded gate in 4.2.2: whether she keeps granting is her judgement, and
        // the engine's own arithmetic is the only mechanical bound (vanilla has
        // no bonus-clearance ceiling of its own - its repair minigames add +1
        // without limit, per the note at AlreadyDone).
        ...
        static bool AlreadyDone(Act a, object beh)
        {
            try
            {
                switch (a.Field)
                {
                    ...
                    case "raise_their_clearance":
                    {
                        // Never "done": clearance is a ladder she may climb with
                        // the player as many times as she chooses. The cap that
                        // used to live here is retired - see the note on
                        // _clearanceGrants for the OOC denial it caused. The
                        // level check still guards the action existing at all.
