    ...
    internal static class Extras
    {
        ...
        static string SummaryPrompt()
        {
            return "You are writing the closing recap on the ending screen of a "
                ...
                + "reached.\n\n"
                + "Write the recap in ONE or TWO SHORT sentences, and NO MORE THAN "
                + "130 CHARACTERS in total. That is not a style preference: the box "
                + "it appears in holds about three short lines and cuts everything "
                + "past them mid-word, so a longer recap is simply not read. One "
                + "vivid short sentence beats two cramped ones. Past tense, "
                + "addressing the player as \"you\", covering how it actually ended. "
                + "Draw only on what is in the record - do not invent events, names "
                + "or details that are not there. " + LanguageRule() + "\n\n"
        ...
        static IEnumerator Run(Kind kind, string json, Action<string> callback)
        {
            ...
            {
                reply = FitBox(TwoSentences(Plain(reply)));
        ...
        // Hard character budget for the recap box.
        //
        // The two-sentence cap alone was not enough, and a real ending screen
        // proved it: the model wrote ONE sentence of 200-odd characters and the
        // box cut it at "your bond with the station". Measured off that
        // screenshot, the box holds about three short lines - roughly 140
        // characters - so the enforced budget is 140 with the prompt aiming at
        // 130 to leave headroom.
        //
        // Cut preference, best to worst: the last sentence end inside the
        // budget; the last word boundary, with an ellipsis; nothing readable
        // fits (pathological) - return the head anyway, since the box will cut
        // it identically no matter what we do.
        const int RecapBudget = 140;

        static string FitBox(string s)
        {
            if (string.IsNullOrEmpty(s) || s.Length <= RecapBudget) return s;

            int atSentence = -1;
            for (int i = 0; i < RecapBudget; i++)
            {
                char c = s[i];
                if (c != '.' && c != '!' && c != '?' && c != '…') continue;
                int end = i;
                while (end + 1 < s.Length && end + 1 < RecapBudget
                    && (s[end + 1] == '"' || s[end + 1] == '\''
                     || s[end + 1] == '”' || s[end + 1] == '’')) end++;
                atSentence = end;
                i = end;
            ...
            if (atSentence > 20)
            {
                string cut = s.Substring(0, atSentence + 1).TrimEnd();
                Plugin.Log.LogInfo("Ending summary cut to the box at a sentence end ("
                    + s.Length + " -> " + cut.Length + " chars).");
                return cut;
            }

            int atWord = s.LastIndexOf(' ', RecapBudget - 2);
            if (atWord > 20)
            {
                string cut = s.Substring(0, atWord).TrimEnd() + "…";
                Plugin.Log.LogInfo("Ending summary cut to the box at a word boundary ("
                    + s.Length + " -> " + cut.Length + " chars).");
                return cut;
            }

            return s.Substring(0, RecapBudget - 1) + "…";
        }

        // Cut the recap to its first two sentences.
        //
        // The prompt asks for at most two, but a screenshot from a real ending
        // showed why the instruction cannot be the only defence: a longer recap
        // is truncated by the SCREEN, mid-word, at whatever character happens to
        // hit the box edge - "your AI assistant who". Two complete sentences the
        // player can read beat five they cannot. Cutting at a sentence boundary
        // is the same reasoning as Schema.LimitReply, and the same cut points:
        // . ! ? and the ellipsis, plus any closing quote that follows.
        //
        // A reply with no second sentence boundary is returned whole - if the
        // model wrote one long sentence there is no safe place to cut it, and
        // the prompt has already done what it can.
        static string TwoSentences(string s)
        {
            ...
            for (int i = 0; i < s.Length; i++)
            {
                ...
                if (sentences == 2)
                {
                    string cut = s.Substring(0, end + 1).TrimEnd();
                    if (cut.Length < s.TrimEnd().Length)
                        Plugin.Log.LogInfo("Ending summary trimmed to two sentences ("
                            + s.Length + " -> " + cut.Length + " chars).");
                    return cut;
                }
                i = end;
