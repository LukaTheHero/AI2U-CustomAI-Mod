...
namespace AI2UCustomAI
{
    internal static class Items
    {
        ...
        public static void Repair(JObject o)
        {
            ...
            // No exact match: before letting it through as an improvised gift,
            // check whether it is a real item's name said in fewer words. The
            // Siren's opening gift found this hole: she wrote "stick" for the
            // level's "Wooden Stick", nothing in the library equals "stick", and
            // the engine minted a generic IsAiGift bag named Stick - wrong
            // artwork, wrong description, and useless for the campfire repair
            // the real item exists for. A library match is strictly better than
            // an improvised one (real sprite, quest checks pass, leaves her
            // stock honestly), so an unambiguous upgrade is always taken.
            if (canonical == null)
            {
                string widened = ResolveShort(name);
                if (widened != null)
                {
                    Plugin.Log.LogInfo("Gift \"" + name + "\" is not a real item but \""
                        + widened + "\" is; giving that instead.");
                    o["giving_to_player"] = widened;
                    return;
                }
            }
        ...
        // "stick" -> "Wooden Stick": whole-word containment in either
        // direction, so "art" can never bleed into "Heart". Her own pockets
        // outrank the level library - when she is holding a Wooden Stick and
        // says "stick", she means the one in her hand - and among equals the
        // longer (more specific) name wins, mirroring Resolve.
        static string ResolveShort(string name)
        {
            string best = null;
            bool bestCarried = false;

            List<string> carried = Carried();
            List<string> pool = new List<string>(carried);
            ...
            for (int i = 0; i < pool.Count; i++)
            {
                ...
                if (string.Equals(cand, name, StringComparison.OrdinalIgnoreCase)) continue;
                if (!WordInside(cand, name) && !WordInside(name, cand)) continue;

                bool isCarried = i < carried.Count;
                if (best == null
                    || (isCarried && !bestCarried)
                    || (isCarried == bestCarried && cand.Length > best.Length))
                {
                    best = cand;
                    bestCarried = isCarried;
                }
            }
            return best;
        }

        // True when needle appears in hay as whole words, not a fragment.
        static bool WordInside(string hay, string needle)
        {
            return (" " + hay.ToLowerInvariant() + " ")
                .IndexOf(" " + needle.ToLowerInvariant() + " ", StringComparison.Ordinal) >= 0;
        }
